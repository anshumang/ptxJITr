...
namespace cuda {
	...
	optionValues[0] = (void*)CU_TARGET_COMPUTE_35; 
	...
	ret = cuGetErrorName(ret, (const char **)&errorStr);
	report("Error name : " << errorStr);
	ret = cuGetErrorString(ret, (const char **)&errorStr);
	report("Error string : " << errorStr);
